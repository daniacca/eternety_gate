...
export function CombatGrid({ containerWidth, containerHeight, combat, save, styles }: CombatGridProps) {
  ...
  const mapWidth = tileSize * grid.width;
  const mapHeight = tileSize * grid.height;
  ...
  return (
    <View style={styles.gameArea}>
      ...
      <View style={localStyles.cameraControls}>
        ...
        <Text style={localStyles.cameraLabel}>{tileSize}px</Text>
        <Pressable
          style={localStyles.cameraButton}
          onPress={() => setTileSize((s) => clamp(snap(s + 8)))}
        >
        ...
        <Pressable
          style={[localStyles.cameraButton, localStyles.cameraButtonSecondary]}
      ...
      <View style={[localStyles.viewport, { width: availableWidth, height: availableHeight }]}>
        ...
        {combat.participants.map((actorId) => {
          ...
          const tokenX = pos.x * tileSize + cellWidth / 2;
          const tokenY = pos.y * tileSize + cellHeight / 2;
