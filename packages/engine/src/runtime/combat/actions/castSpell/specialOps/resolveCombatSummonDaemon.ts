...
export function resolveCombatSummonDaemon(params: SpecialOpParams): SpecialOpResult | null {
  const { save, catalogs, combat, rng, turnActorId, spell, effectDef, effectStatBonus, getOvercastForTarget, terrainContentPack } =
    params;
  if (effectDef.specialOp !== "combatSummonDaemon") {
    return null;
  }
  ...
  const summonId = `SUMMON_DAEMON_${turnActorId}_${seq}`;
  ...
  const daemonSpells = getAllSpells()
    .filter((spellDef) => spellDef.discipline === "DAEMONOLOGY")
    .reduce<Record<string, boolean>>((acc, spellDef) => {
      acc[spellDef.id] = true;
      return acc;
    }, {});

  daemonSpells["spell:flame_bolt"] = true;
  daemonSpells["spell:flame_cone"] = true;
  daemonSpells["spell:pyra_explosion"] = true;
  ...
  const summonActor = buildSummonedBaseActor({
    ...
    traits: {
      ...
      "trait:daemonic": { x: s },
      "trait:deadly_natural_weapons": true,
      "trait:warp_weapons": true,
  ...
  if (!summonPos) {
    updatedSave = appendCombatLog(updatedSave, "Non c'è spazio per evocare il demone.");
  ...
  updatedSave = appendCombatLog(updatedSave, "Evoci un Demone.");
  if (casterIsParty) {
