...
import { combatStandUp } from "./standUp";
...
describe("combatStandUp", () => {
  it("should remove prone condition and consume all movement", () => {
    ...
    const effect: Effect = { op: "combatStandUp", actorId: "PC_1" };
    const result = combatStandUp(effect as Extract<Effect, { op: "combatStandUp" }>, saveWithMovement);
  ...
  it("should return unchanged save if combat is not active", () => {
    ...
    const effect: Effect = { op: "combatStandUp", actorId: "PC_1" };
    const result = combatStandUp(effect as Extract<Effect, { op: "combatStandUp" }>, save);
  ...
  it("should return unchanged save if not actor's turn", () => {
    ...
    const effect: Effect = { op: "combatStandUp", actorId: "PC_1" };
    const result = combatStandUp(effect as Extract<Effect, { op: "combatStandUp" }>, npcTurnSave);
  ...
  it("should return unchanged save if no movement remaining", () => {
    ...
    const effect: Effect = { op: "combatStandUp", actorId: "PC_1" };
    const result = combatStandUp(effect as Extract<Effect, { op: "combatStandUp" }>, saveWithNoMovement);
  ...
  it("should add narration for PC actor", () => {
    ...
    const effect: Effect = { op: "combatStandUp", actorId: "PC_1" };
    const result = combatStandUp(effect as Extract<Effect, { op: "combatStandUp" }>, combatSave);

    const lastLog = result.save.runtime.combatLog?.[result.save.runtime.combatLog.length - 1];
    expect(lastLog).toContain("Ti alzi in piedi");
  });
});
