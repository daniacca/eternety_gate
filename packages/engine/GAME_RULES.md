...
- **Resist spells (formerly opposed)**: Spells that require the target to resist (e.g. mental control, disarm, daemonbane) work as follows. The caster rolls for **cast success** as usual (CN, PM, cast modifier). If the cast succeeds, the **target** rolls a single **resist check** (stat and difficulty from the spell). The resist check modifier is: **base penalty** (from spell power, or by baseCN band: 0–1 → 0, 2–4 → −10, 5–7 → −20, 8–10 → −30, 10+ → −40) **−10× overcast** + MR bonus + Untouchable. The effect **lands if and only if the target fails** the resist check (no comparison with caster DoS). Effect magnitude (damage, duration, etc.) still scales with the target's **DoF** on the resist roll and with overcast/effectStat.
- **Blessing and buff spells**: Spells that apply beneficial conditions or temporary modifiers (e.g. force field, steel body, sanctuary) target self and/or allies. **Duration** and **stacks** (or magnitude) use a **base from the spell's baseCN** (higher CN → higher base, since overcast is limited at high CN) plus **effectStat** and **overcast**. Formula: duration = baseDuration(baseCN) + effectStatBonus + overcast; stacks = baseStacks(baseCN) + overcast. Base stacks by baseCN (so high-CN spells stay meaningful despite less overcast): 0–1→1, 2–4→3, 5–7→5, 8–10→6, 10+→7. Effects with **aura** (e.g. sanctuary, word of god) propagate in radius; when multiple auras overlap the same target, the **stronger aura wins**. **Aura power** = **emitted MC + floor(DoS/2)** (magic power dominates; DoS is a small tie-breaker so that at equal MC the caster with better control wins). **durationRounds: 0** in effect data means “use default base from baseCN and scale with overcast”.
